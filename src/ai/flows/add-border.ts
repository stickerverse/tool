  ...
  async (input) => {
    const {media, finishReason} = await ai.generate({
      prompt: [
        ...
        {text: `**Do not generate a new image.** You must only edit the provided image. Add a solid, sticker-style border that follows the exact contour of the subject in the provided image. The image already has a transparent background. The border should be ${input.borderWidth} pixels wide and have the color ${input.borderColor}. The final output must be a PNG image with only the subject and its new border visible on a transparent background.`},
      ],
